...
const sistemas: SistemaData[] = [
  {
    ...
    label: "Mi Tienda",
    icon: <StorefrontIcon />,
    empresas: [8, 12, 15, 20, 24, 28, 33, 38, 41, 45, 49, 53],
    ingresos: [
      4200, 5800, 6900, 8200, 10500, 12100, 13900, 15200, 16900, 18400,
      19800, 21600,
    ],
    vencidas: 15,
  },
  {
    ...
    empresas: [3, 5, 8, 11, 14, 18, 22, 26, 31, 35, 39, 44],
    ingresos: [
      2500, 3900, 5200, 7100, 8800, 11200, 13800, 15900, 18300, 20700,
      22900, 25400,
    ],
    vencidas: 8,
  },
...
function getTotalEmpresasSistema(sistema: SistemaData) {
  return getUltimoValor(sistema.empresas);
}

function getVencidasSistema(sistema: SistemaData) {
  const total = getTotalEmpresasSistema(sistema);

  if (sistema.vencidas > total) {
    return total;
  }

  return sistema.vencidas;
}

function getActivasSistema(sistema: SistemaData) {
  const total = getTotalEmpresasSistema(sistema);
  const vencidas = getVencidasSistema(sistema);

  return Math.max(total - vencidas, 0);
}
...
export default function Metricas() {
  ...
  const totalEmpresas = useMemo(() => {
    return sistemasFiltrados.reduce(
      (total, sistema) => total + getTotalEmpresasSistema(sistema),
      0
    );
  }, [sistemasFiltrados]);
  ...
  const totalActivas = useMemo(() => {
    return sistemasFiltrados.reduce(
      (total, sistema) => total + getActivasSistema(sistema),
      0
    );
  }, [sistemasFiltrados]);
  ...
  const totalVencidas = useMemo(() => {
    return sistemasFiltrados.reduce(
      (total, sistema) => total + getVencidasSistema(sistema),
      0
    );
  }, [sistemasFiltrados]);
  ...
  const ingresosDelMes = useMemo(() => {
    return sistemasFiltrados.reduce(
      (total, sistema) => total + getUltimoValor(sistema.ingresos),
      0
    );
  }, [sistemasFiltrados]);
  ...
  const crecimientoAnual = useMemo(() => {
    return sistemasFiltrados.reduce((total, sistema) => {
      const primero = sistema.empresas[0] ?? 0;
      const ultimo = getTotalEmpresasSistema(sistema);

      return total + (ultimo - primero);
    }, 0);
  }, [sistemasFiltrados]);
  ...
  const cards = [
    ...
    },
    {
      ...
      value: formatMoney(ingresosDelMes),
      ...
      >
        <Box>
          ...
          <Typography color="text.secondary" mt={1}>
            Comparación mensual entre Mi Tienda y ClicMenu.
          </Typography>
        </Box>
        ...
        <Grid item xs={12} lg={4}>
          ...
          >
            ...
            <Stack spacing={2}>
              ...
              {sistemasFiltrados.map((sistema) => {
                const total = getTotalEmpresasSistema(sistema);
                const activas = getActivasSistema(sistema);
                const vencidas = getVencidasSistema(sistema);
                const percent = total > 0 ? (activas / total) * 100 : 0;
                      ...
                      <Stack direction="row" alignItems="center" spacing={1}>
                        ...
                        <Box>
                          ...
                          <Typography fontSize={11} color="text.secondary">
                            {activas} activas / {vencidas} vencidas / {total} total
                          </Typography>
                        </Box>
                      </Stack>
        ...
        <Grid item xs={12}>
          ...
          >
            ...
            >
              ...
              <Chip
                icon={<ReceiptLongIcon />}
                label={formatMoney(ingresosDelMes)}
                size="small"
                sx={{ fontWeight: 800 }}
              />
            </Stack>
                ...
                <Grid item xs={12} md={6} key={sistema.key}>
                  ...
                  >
                    ...
                    >
                      ...
                      <Typography fontWeight={900}>
                        {formatMoney(getUltimoValor(sistema.ingresos))}
                      </Typography>
                    </Stack>
